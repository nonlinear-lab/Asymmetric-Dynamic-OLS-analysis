...
results_slope <- matrix(NA, nrow = reps, ncol = 7)
colnames(results_slope) <- c("OLS", "DOLS", "Fourier DOLS","FMOLS", "Fourier_FMOLS", "Asymmetricr_DOLS_plus", "Asymmetricr_DOLS_minus")
...
for (i in 1:reps) {
  ...
  # 6. Asymmetric DOLS (from dols_method.R)
  asym_res <- asymmetric_dols_manual(y, x, n_leads, n_lags)
  results_slope[i, 6] <- asym_res$beta_plus
  results_slope[i, 7] <- asym_res$beta_minus
